...
parfor i = 1:num_a_c
    ...
    for j = 1:num_o_c
        ...
        [k, ~, res] = AGADI_HS(A, b, alpha, omega, tol, maxit, x_init);
        temp_iter(j)= k;
        temp_res(j) = res(end);
    end
...
% 美化整体排版
sgtitle('FKM\_ADI: Two-Stage grid search optimization results analysis for n = '+string(n),  'FontSize', 14, 'FontWeight', 'bold');