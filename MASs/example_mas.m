clear; clc;
N = 4;
k1=1;k2=0.5;k3=0.8;k4=2;
m1=8e-3;m2=9e-3;m3=6e-3;m4=9e-3;
A{1} = [0, 1; -k1/m1, 0];  B{1} = [0; 1/m1];  C{1} = [1, 0];
A{2} = [0, 1; -k2/m2, 0];  B{2} = [0; 1/m2];  C{2} = [1, 0];
A{3} = [0, 1; -k3/m3, 0];  B{3} = [0; 1/m3];  C{3} = [1, 0];
A{4} = [0, 1; -k4/m4, 0];  B{4} = [0; 1/m4];  C{4} = [1, 0];
...
A_adj = [0 1 0 0; 0 0 0 1; 1 0 0 1; 0 0 1 0];  
G = diag([1, 1, 0, 0]); 
c = 0.5;  
dt = 1;  
...
for i = 1:N
    ...
    for k = k_values
        [X_k, Y_k, u_k, e_k, R0_k] = compute_states_error(A_bar, B_bar, C_bar, K_iter{i}, X_0, k, A_adj, G, c, F, T,dt, i, A);

        R0_k_agents = reshape(R0_k(1:N*p), p, N);
        y0_i_k = T * R0_k_agents(:, i);  
     

        e_k = norm(Y_k(1:end-1, :) - y0_i_k, 2);
